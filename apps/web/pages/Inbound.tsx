...
import { fetchInboundOrders, fetchInboundOrderDetails } from '../services/api';
...
const Inbound: React.FC = () => {
  ...
  const [activeOrderDetails, setActiveOrderDetails] = useState<InboundOrder | null>(null);
  ...
  useEffect(() => {
    const loadOrders = async () => {
        try {
            ...
            setOrders(data);
            if (data.length > 0) setActiveOrderId(data[0].id);
        } catch (e) {
            console.error("Using Mock Data for Orders", e);
            setOrders(INBOUND_ORDERS);
            setActiveOrderId(INBOUND_ORDERS[0].id);
        } finally {
            setIsLoading(false);
        }
    };
  ...
  useEffect(() => {
      ...
      const loadDetails = async () => {
          try {
             // Try fetching full details if the list didn't include items
             const details = await fetchInboundOrderDetails(activeOrderId);
             setActiveOrderDetails(details);
          } catch(e) {
              // Fallback to finding in the current list if detail fetch fails (or mock)
              const found = orders.find(o => o.id === activeOrderId);
              if (found) setActiveOrderDetails(found);
          }
      };
      loadDetails();
