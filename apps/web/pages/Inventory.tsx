...
import { getInventoryWithDetails } from '../services/mockData'; // Fallback
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Manual join for display since backend returns raw FKs in this simple MVP
        const enrichedInventory = invData.map(item => ({
            ...item,
            product: prodData.find(p => p.id === item.product_id),
            // Location is not fetched separately in this basic version, relying on what's in item or basic fallback
            // In a real app we'd fetch locations or include them in the JOIN query in backend
            location: { id: item.location_id, name: item.location_id === 'l4' ? 'Receiving' : 'Storage', type: 'storage', max_volume: 100 } as any 
        }));

        setData(enrichedInventory);
      } catch (e) {
        console.error("Failed to load from API, falling back to mock", e);
              ...
              {data.map((item) => (
                <React.Fragment key={item.id}>
                  ...
                  >
                    ...
                    <td className="px-6 py-3 text-slate-600">{item.location_id}</td>
            ...
            <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
               ...
               <div className="flex justify-between items-center text-sm border-t border-slate-100 pt-3">
                   ...
                   <div className="flex flex-col text-left">
                       ...
                       <span className="font-medium bg-slate-100 px-2 py-0.5 rounded text-slate-700">{item.location_id}</span>
                   </div>
               </div>
            </div>
          ))}
